...
uniform bool  u_UseEnvReflection;
...
// Inline procedural sky — identical values to Skybox.shader
vec3 SampleEnvironment(vec3 dir)
{
    vec3  d       = normalize(dir);
    float t       = clamp(d.y * 0.5 + 0.5, 0.0, 1.0);
    vec3  zenith  = vec3(0.08, 0.20, 0.55);
    vec3  horizon = vec3(0.55, 0.72, 0.90);
    vec3  ground  = vec3(0.18, 0.15, 0.12);
    if (d.y < 0.0)
...
void main()
{
    ...
    if (u_UseEnvReflection)
    {
        vec3  R        = reflect(-V, N);
        vec3  envColor = SampleEnvironment(R);
        vec3  F_env    = FresnelSchlick(max(dot(N, V), 0.0), F0);
        float weight   = (1.0 - u_Roughness) * u_EnvReflectionStrength;
        envContrib     = F_env * envColor * weight;
    }

    vec3 color = ambient + Lo + envContrib;
